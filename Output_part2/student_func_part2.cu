                       ...
                       int numRows, int numCols)
{
        ...
        uchar4 rgba = rgbaImage[i+ j*numRows];//rgba values of the pixel to be processed 
        float value = ((max(max(rgba.x, rgba.y), rgba.z)) + min(min(rgba.x, rgba.y),rgba.z))/2; //formula
        greyImage[i + j*numRows] = (unsigned char)value; //storing the output value to the corresponding pixel of the grey image
